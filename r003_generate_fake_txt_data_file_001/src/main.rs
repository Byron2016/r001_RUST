/*
- Generating data 100x faster with Rust — Part II
    - https://itnext.io/generating-data-100x-faster-with-rust-part-ii-49eaa0f388af
        - https://github.com/aprxi/faster-data-generation/tree/main/examples/rsfake-v2

    - 
        # compile and test (fast build, slow execution)
        cargo run -- -s schema.json

        # compile for release (slow build, fast execution)
        cargo build --release
        target/release/r003_generate_fake_txt_data_file_001 -s schema.json

    - 
        # compile for release
        cargo build --release

        # generate million rows on single thread (default)
        target/release/r003_generate_fake_txt_data_file_001 -r 1000000

        # generate on 2, 4, 8 threads
        target/release/r003_generate_fake_txt_data_file_001 -r 1000000 -t 2
        target/release/r003_generate_fake_txt_data_file_001 -r 1000000 -t 4
        target/release/r003_generate_fake_txt_data_file_001 -r 1000000 -t 8
*/
